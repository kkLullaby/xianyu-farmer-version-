...
报价：¥${item.price}元/斤，重量：${item.weight}斤`,
        success: (res) => {
          ...
          const newOrder = {
            id: "ORD" + Date.now(),
            order_no: "ORD-" + (/* @__PURE__ */ new Date()).toISOString().slice(0, 10).replace(/-/g, "") + "-" + String(orderList.length + 1).padStart(3, "0"),
            intention_id: item.id,
            farmer_name: item.sender_name || "农户用户",
            variety: item.variety || "柑橘果肉",
            weight: item.weight,
            unit_price: item.price,
            total_price: (Number(item.price) * Number(item.weight)).toFixed(2),
            address: item.address || "待确认",
            expected_delivery: item.date || "待协商",
            flow_status: "pending_ship",
            status: "待接单",
            created_at: (/* @__PURE__ */ new Date()).toLocaleString("zh-CN").replace(/\//g, "-"),
            timeline: [
              { time: (/* @__PURE__ */ new Date()).toLocaleString("zh-CN").replace(/\//g, "-"), desc: "意向被接受，订单已创建" }
            ]
          };
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        f: common_vendor.f(common_vendor.unref(displayList), (item, index, i0) => {
          return common_vendor.e({
            a: common_vendor.t(item.sender_name || "农户用户"),
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-d8da9379"], ["__file", "C:/Users/lkf12/WeChatProjects/project-recycle/xianyu-farmer-version-/src/pages/merchant/intentions/index.vue"]]);
