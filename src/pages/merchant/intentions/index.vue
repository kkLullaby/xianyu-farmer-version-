...
const handleAccept = (item) => {
  uni.showModal({
    ...
    success: (res) => {
      ...
      const newOrder = {
        id: 'ORD' + Date.now(),
        order_no: 'ORD-' + new Date().toISOString().slice(0, 10).replace(/-/g, '') + '-' + String(orderList.length + 1).padStart(3, '0'),
        ...
        timeline: [
          { time: new Date().toLocaleString('zh-CN').replace(/\//g, '-'), desc: '意向被接受，订单已创建' }
        ]
      };
