...
CREATE TABLE IF NOT EXISTS cms_announcements (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    type TEXT NOT NULL,              -- policy(政策), subsidy(补贴), platform(平台公告)
    title TEXT NOT NULL,
    summary TEXT,
    image_url TEXT,                  -- 图片路径
    link_url TEXT,                   -- 链接
...
CREATE TABLE IF NOT EXISTS cms_cases (
    ...
    buyer_name TEXT,
    seller_name TEXT,
    trade_data TEXT,                 -- 交易数据描述
    thumbnail_url TEXT,              -- 缩略图
    logo_url TEXT,                   -- 企业logo
    sort_order INTEGER DEFAULT 0,
    is_active INTEGER DEFAULT 1,
    created_by INTEGER,
    created_at DATETIME DEFAULT (datetime('now')),
...
CREATE TABLE IF NOT EXISTS cms_site_info (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    info_key TEXT NOT NULL UNIQUE,   -- phone, license, about, etc.
    info_value TEXT,
    updated_by INTEGER,
    updated_at DATETIME DEFAULT (datetime('now'))
);
